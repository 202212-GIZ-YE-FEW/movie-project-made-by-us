  ...
  movies.map((movie) => {
    ...
    movieDiv.innerHTML = `<div class="card">
        ...
        .join(', ')}</p>
					...
					<h3 class="card card-subtitle text-center fs-4 fw-bold">${movie.title || movie.name
      }</h3>
					<p class="rating fw-bold text-center fs-3">Average: ${movie.vote_average
...
const renderPersons = (persons) => {
  ...
  persons.map((person) => {
    ...
    personDiv.innerHTML = `
	  ...
	  <img class="card-img-top"src="${PROFILE_BASE_URL + person.profile_path
      }" alt="${person.name}">
	  <div class="card-body">
	  <h3>${person.name}</h3>
